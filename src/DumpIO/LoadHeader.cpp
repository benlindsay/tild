  ...
  while (line.find("ITEM: ATOMS") == std::string::npos) {
    ...
    if (line.find("ITEM: BOX BOUNDS") != std::string::npos) {
      ...
      while (line.find("ITEM:") == std::string::npos) {
        ...
        if (bb_t_i[1] - bb_t_i[0] > 0.0) {
          // Only add to dimensionality if box width is nonzero. This allows
          // dummy box bounds of 0.0 and 0.0 to be there without requiring a z
          // column of data. 3 box bounds are required in Ovito even for 2D
          // simulations.
          d++;
